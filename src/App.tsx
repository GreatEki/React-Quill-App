...
import { Options, ModalKeys } from "./interface";
...
export default function App() {
  ...
  const toggleModal = (modalName: ModalKeys) => {
  ...
  function extractVideoId(url: string) {
    ...
    const match = url.match(pattern) || url.match(vimeoPattern);
  ...
  const handleEmbedVideo = () => {
    ...
    if (links.type?.includes("youtube")) {
      videoEmbed = `<iframe src='https://www.youtube.com/embed/${videoId}' frameborder='0' allowfullscreen></iframe>`;

      setContent(videoEmbed);
    }

    if (links.type?.includes("vimeo")) {
      videoEmbed = `<iframe src='https://player.vimeo.com/${videoId}' frameborder='0' allowfullscreen></iframe>`;
    }
